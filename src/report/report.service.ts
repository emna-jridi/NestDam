...
export class ReportService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey && apiKey.startsWith('AIza')) {
      ...
      this.logger.log(
        `✨ Gemini AI Client Ready with model: ${this.modelName}`,
      );
    } else {
      this.logger.warn(
        '⚠️ No valid API Key found. Defaulting to Local Engine.',
      );
      this.modelName = '';
    }
  }
  ...
  async generateSafetyReport(packageName: string): Promise<ReportDto> {
    ...
    if (this.genAI && this.modelName) {
      try {
        ...
        this.logger.error(
          `❌ AI analysis failed: ${error.message}. Switching to Local Engine.`,
        );
      }
    }
...
{"appName":"App Name","riskLevel":"High|Medium|Low","dataPrivacy":"One sentence risk summary","recommendations":["Tip 1","Tip 2","Tip 3"]}`;
    ...
    this.logger.log(
      `✅ Success! Analyzed ${packageName} using ${this.modelName}`,
    );
  ...
  private analyzeLocally(packageName: string): ReportDto {
    ...
    if (
      ...
      report.dataPrivacy =
        'Financial App: Handles sensitive banking credentials.';
      ...
      packageName.includes('social') ||
      packageName.includes('chat') ||
      packageName.includes('gram') ||
      packageName.includes('book')
    ) {
      report.riskLevel = 'Medium';
      report.dataPrivacy =
        'Social App: Likely collects contacts, location, and usage data.';
      report.recommendations = [
      ...
      packageName.includes('puzzle') ||
      packageName.includes('io')
    ) {
